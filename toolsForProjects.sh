    ...
    if [ -n "$port_project" ]&&[ -n "$name_project" ]; then
        ...
        if [ "$is_continue" = "y" ]; then
            ...
            # NGinx
                ...
                local is_port_exist="$(sudo lsof -i -n | grep nginx | awk '{print $9}' | cut -d ':' -f2 | grep -c "$port_project")"
                local template_nginx="$filesTemplatePath"
                ...
                if [ "$is_continue" = "y" ]; then
                    ...
                    if [ "$tecnology_selected" = "1" ]; then
                        template_nginx="$template_nginx/PHP/virtualConfNginxTemplate"
